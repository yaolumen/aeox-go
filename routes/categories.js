...
router.post('/config/categories', adminSession, validate(CategoryCreateSchema), async (req, res) => {
  const { name, slug, description, sort } = req.body;
  await withLock('categories.json', async () => {
    const cats = await loadJson('categories.json', []);
    const finalSlug = slug || generateSlug(name.en || name.zh || name.es || name.de || name);
    if (cats.some((c) => c.slug === finalSlug)) return res.status(409).json({ error: 'slug 已存在' });
    const cat = {
      id: generateId('cat'),
      slug: finalSlug,
      name: typeof name === 'string' ? { en: name, zh: name, es: name, de: name } : { en: name?.en || '', zh: name?.zh || '', es: name?.es || '', de: name?.de || '' },
...
router.put('/config/categories/:id', adminSession, validate(CategoryUpdateSchema), async (req, res) => {
  const { id } = req.params;
...
router.delete('/config/categories/:id', adminSession, async (req, res) => {
  const { id } = req.params;
  await withLock('categories.json', async () => {
    const cats = await loadJson('categories.json', []);
